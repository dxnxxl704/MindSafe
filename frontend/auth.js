...
function initials(name = 'MS') {
    return name.split(' ').filter(Boolean).slice(0, 2).map((part) => part[0].toUpperCase()).join('') || 'MS';
}
...
function validateRequiredFields(form) {
    ...
    const emptyField = requiredFields.find((field) => field.type === 'checkbox' ? !field.checked : !field.value.trim());
    if (emptyField) {
        emptyField.focus();
...
function setupRegistration() {
    ...
    form.addEventListener('submit', (event) => {
        ...
        const confirmation = form.querySelector('#register-password-confirmation').value;
        if (password.length < 6) { setFormMessage('A senha deve ter pelo menos 6 caracteres.', true); return; }
        if (password !== confirmation) { setFormMessage('As senhas precisam ser iguais.', true); return; }
        const user = {
        ...
        window.setTimeout(() => { window.location.href = 'perfil.html'; }, 700);
    });
}
...
function fillProfile(user) {
    ...
    document.querySelector('#profile-name-heading').textContent = user.name || 'Seu perfil';
    document.querySelector('#profile-email-heading').textContent = user.email || 'Adicione seu e-mail';
